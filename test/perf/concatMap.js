...
suite
    ...
    .add('rx 5', function(deffered) {
        runners.runRx5(deffered, rxjs.Observable.from(a).concatMap(function(x) {
    ...
    .add('lodash', function(deferred) {
        runners.runLodash(deferred, function(a,cb) {
            cb( lodashConcatMap(identity, a).reduce(sum, 0) );
        });
    })
    .add('array', function(deferred) {
        runners.runArray(deferred, function(a,cb) {
            cb( arrayConcatMap(identity, a).reduce(sum, 0) )
        });
    });
